...
## 背景繪製進入點
- [x] `sprites.ts` 新增 `drawMapBackground(g, kind, cx, cy, viewW, viewH, clock, gridColor, gridAlpha)`
- [x] `PixiRenderer.render()` 改呼叫 `drawMapBackground`，不再直接呼叫 `drawBackgroundGrid`
- [x] `drawBackgroundGrid` 簽章不變、被 `drawMapBackground` 內部沿用
...
## 地面特徵層（依 kind）
- [x] plains：淡綠草叢（短線）+ 零星小石
- [x] lava：暗色岩裂折線 + 餘燼亮點
- [x] tundra：淺藍冰裂折線 + 雪堆塊
- [x] 只繪製可視範圍內格子（沿用視窗裁切迴圈）

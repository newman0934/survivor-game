...
### FR-1 背景繪製進入點
- 在 `sprites.ts` 新增 `drawMapBackground(g, kind, cx, cy, viewW, viewH, clock)`：
  依 `kind` 畫「地表色斑層 + 地面特徵層 + 氛圍動畫層」（**不畫網格**）。
- 取代 `PixiRenderer.render()` 目前對 `drawBackgroundGrid` 的直接呼叫。
- `drawBackgroundGrid` 保留為共用底層工具（不再被背景使用，留作未來備用）。

### FR-1b 地表色斑層（依 kind，世界座標、隨玩家捲動）
- 新增私有 `groundPatches`：大格點（如 220）疊三層同心、低透明度的柔和色斑，
  製造地表起伏暈染，取代網格提供空間質感。
- 色系依 kind（`PATCH_COLORS`）：plains 深淺綠、lava 暗紅、tundra 深藍。
- 位置/大小/色選同樣由 `bgHash` 決定 → 捲動穩定重現。
...
- 用視窗裁切迴圈只走可視範圍內的格子（格距 128）。
- 每格用 `bgHash` 決定是否繪製及其外觀，同格永遠相同 → 捲動穩定重現。
- **plains**：雙色草叢（暗綠底 + 亮綠覆蓋的多筆短線）與立體碎石（落地影 + 高光）。
- **lava**：岩裂折線（深色陰影底 + 橙紅內光雙線）與餘燼亮點（外發光暈 + 亮核）。
- **tundra**：冰裂折線（暗藍底 + 亮藍高光雙線）與雙層柔化雪堆。
...
- 底色仍走既有 `world.mapBgColor`；`mapGridColor/mapGridAlpha` 保留於 MapDef（背景不再使用）。
...
export function drawMapBackground(
  ...
  viewW: number, viewH: number, clock: number,
): void
export function bgHash(gx: number, gy: number): number // [0,1) 純函式
// drawBackgroundGrid 簽章不變（保留備用，背景不再呼叫）

...
## 狀態：階段 0 + 1 完成 ✅，階段 2 進行中（多種武器 ✅、多種敵人 ✅、Boss ✅）

引擎地基 + 完整可玩的核心循環，並已加入**多種武器系統**、**多種敵人系統**與**Boss 敵人**。
已在瀏覽器端到端實測（選單 → 遊玩 → 自動攻擊 → 擊殺 → 撿寶石 → 升級彈窗〔解鎖新武器/升級武器/被動〕
→ 強化 → 恢復 → 死亡結算 → 重新開始），四把武器可共存同時開火，四種敵人依時間解鎖加權登場，
Boss 每 60s 出現並顯示專屬血條，
並且 **83 個單元測試全過、型別檢查乾淨、production build 乾淨、無功能相關 console error**。
...
| 型別檢查（vue-tsc） | 乾淨 |
| Production build | 乾淨 |
| 瀏覽器煙霧測試 | 階段 1 + 多種武器 + 多種敵人 + Boss 所有驗收標準通過；唯一 console error 為既有 favicon 404（與功能無關） |

...
「**免疫大戰**」主題（免疫細胞 vs 病原體，場景在血管／胃／肺泡）：7 武器、8 病原（含遠程／分裂／自爆）、
Boss、10 被動、4 角色、3 地圖；進度存檔記錄戰績與累積統計，主選單顯示統計與排行榜，結算可「再玩一次」或
「回主選單」重選。**154 測試全綠、型別檢查乾淨、production build 乾淨、無功能相關 console error。**
...
| 型別檢查（vue-tsc） | 乾淨 |
| Production build | 乾淨 |
| 瀏覽器煙霧測試 | 階段 1–3 + 美術 + 特效 + 新武器/敵種 + 進度存檔 + 排行榜 + 手機 + 音效 驗收通過（偶見既有 favicon 404，與功能無關） |
